...
const CardViewDetail: React.FC<CardViewDetailProps> = ({
  ...
  const getDistricts = (provinceCode: string) => {
    ...
    return [{ value: '', label: 'Select District' }, ...sortOptions(options)]
  }
  ...
  const handleUpdate = (values: updateFormProps) => {
    updateEvent({
      ...
      eventStartTime: values.eventStartTime
        ...
        : undefined,
      eventEndTime: values.eventEndTime ? dayjs(values.eventEndTime) : undefined
    })
    setIsEditing(false)
    onClose()
  }
  ...
  useEffect(() => {
    ...
    form.setFieldsValue({
      ...
      province: event.province ? event.province : undefined,
      district: event.district ? event.district : undefined,
      ward: event.ward ? event.ward : undefined
    })
